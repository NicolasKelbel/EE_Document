...
\begin{figure}[h!]
	...
	\begin{forest}
		for tree={
			...
			edge={draw,-stealth},
			inner sep=2pt,
			l sep=25pt,   % increased horizontal spacing
			s sep=20pt    % increased vertical spacing
		}
		[project-root/
		[0/
		[p]
		...
		[3faces.stl]
		[4faces.stl]
		[\textcolor{blue}{6faces.stl}, label=right:{\footnotesize modified}]
		]
		]
		[system/
		[blockMeshDict]
		[controlDict]
		[\textcolor{red}{snappyHexMeshDict}, label=right:{\footnotesize modified}]
		]
		[para.foam]
		]
	\end{forest}
	
	\caption{Overview of the simulation directory structure. Highlighted files were modified during the project.}
\end{figure}

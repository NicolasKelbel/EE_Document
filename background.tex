...
\end{equation}
where $U$ is the free-stream velocity (\si{\meter\per\second}), $L$ is the characteristic length (\si{\meter}), and $\nu$ is the kinematic viscosity (\si{\meter\squared\per\second}) \parencite{noauthor_relationship_nodate}. This number allows one to classify whether a flow is laminar or turbulent, with low Reynolds numbers signifying laminar flow and high Reynolds number indicating turbulent flow, though the exact threshold values vary between sources \parencite{saldana2024_reynolds}. 
